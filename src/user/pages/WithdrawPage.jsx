...
export default function WithdrawPage() {
  ...
  const isWithdrawalBlocked =
    Boolean(maintenance?.is_enabled) || maintenance?.features?.withdrawals === false;
    ...
    <div data-testid="withdraw-page">
      ...
      {isWithdrawalBlocked && (
        <div
          ...
          data-testid="withdrawal-disabled-banner"
        >
          <AlertTriangle className="h-4 w-4 shrink-0 text-amber-400 mt-0.5" />
          <div>
            <div className="font-semibold text-amber-300">Withdrawals Temporarily Paused</div>
            <div className="text-amber-200/80 mt-0.5">
              {maintenance?.message || "USDT withdrawals are temporarily disabled for scheduled maintenance."}
            </div>
          </div>
        </div>
      )}
